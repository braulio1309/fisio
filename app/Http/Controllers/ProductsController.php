...
class ProductsController extends BaseController
{
    ...
    public function newProducts()
    {
        $dataProducts = Product::
                            ...
                            ->orderBy('id', 'desc')
                            ->limit(20)
                            ->get(['id', 'category_id', 'name', 'cost', 'TaxNet', 'image', 'stock_alert', 'note'])
                            ->each(function($q) {
                                $q->image = explode(",", $q->image);
                            });

        // Url images
            $urlImage = \Request::root().'/images/products';
        // Return response
            return response()->json(
                                [
    ...
    public function productDetails($id)
    {
        $product = Product::
                        ...
                        ->select('id', 'category_id', 'name', 'cost', 'TaxNet', 'image', 'stock_alert', 'note')
                        ->whereId($id)
                        ->first();
        // Explode images
    ...
    public function addFavoriteProducts(Request $request)
    {
        ...
        if ($validate->fails()) {
            return response()->json("Datos faltantes");
        }
        else {
            $add = FavoriteProduct::create([
    ...
    public function listFavoriteProducts($userId)
    {
        ...
        $query->with('product', function($q) {
            $q->select('id', 'name', 'cost', 'image');
        });

        $data = $query->get(['id', 'product_id']);



        // $data = FavoriteProduct::query()->where('user_id', '=', $userId);
        // $data = Product::
        //             query()
        //             ->with('category', function($q) {
        //                         $q->select('id', 'name');
        //                     },
        //                     'favoriteProducts', function($query) use($userId) {
    ...
    public function allProducts(Request $request)
    {
        ...
        if ($request->has('opt'))
        {
            ...
            if ($request->opt === 'low-high') {
                $query->orderBy('cost', 'ASC');
            }
            if ($request->opt === 'high-low') {
